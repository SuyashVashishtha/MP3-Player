...
def prev_music():
        global prev
        global indexp
        global filetoreadprv



        if prev == False:
            play_music()
            indexp = selected_song
            indexp -=1
            filetoreadprv = playlist[indexp]
            mixer.music.load(filetoreadprv)
            mixer.music.play()
            show_detailsn(filetoreadprv)
            statusbar['text']= "Current playing song --" + '' + os.path.basename(filetoreadprv)
